...
async function extractEntry(
  ...
  const view = new DataView(
    ...
    Math.min(data.byteLength - localHeaderOffset, 30),
  );
...
export async function extractZipImages(file: Blob): Promise<ArchiveImage[]> {
  ...
  for (let index = 0; index < entriesCount; index++) {
    ...
    // 跳过目录项
    const isDirectory =
      entryName.endsWith('/') || entryName.endsWith('\\') || (uncompressedSize === 0 && entryName === '');
    if (safeName && !isDirectory) {
      const content = await extractEntry(data, localHeaderOffset, flags, method, compressedSize, uncompressedSize);
      results.push({
        name: safeName,
