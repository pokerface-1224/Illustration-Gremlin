...
- 点击「插图更新」并选择新的 zip 图包，选择后会自动开始更新
- 更新时只覆盖与现有图片**完全同名**的文件（如 `happy.png` 覆盖 `happy.png`），图包未包含的旧图会保留
- 图包中的新文件会正常加入当前角色，普通「导入图包」逻辑不变，仍会对重名文件自动追加序号
...
- 面板中点击「清空插图」，二次确认后即可删除当前角色卡的全部插图（无需逐张删除）
- 确认弹窗会显示角色名与当前张数；群聊场景下清空的是该群名称目录下的插图

### 全部角色插图管理

- 点击「管理全部插图」打开全屏管理页，可查看/管理所有已导入插图的角色目录，**无需打开对应角色卡**
- 管理页列出每个角色的插图张数，进入后支持预览灯箱、单张删除、勾选批量删除、整角色清空与图片重命名
- 重命名可直接修正 `${图片名}` 占位符的匹配名称；同名或非法文件名会被拒绝
- 所有删除/清空操作均有二次确认，操作后聊天中的占位符会立即按最新图片状态重新解析
...
- **角色隔离**：接口只能访问“当前打开的角色卡”的插图——列出当前角色图片、
  按相对路径精确取图、或按文件名在当前角色目录内查找；传入其他角色名会被拒绝（返回 `[]` / `null`），
  角色卡 HTML 前端无法读取其他角色卡的图片
- 聊天中的 `${图片名}` 占位符是插件内部替换，仍按原有规则在所有角色目录中查找，不受上述接口隔离影响
...
1. 打开一个角色卡，展开扩展面板「插图精灵」。
2. 选择 zip 压缩包（可多选），选择后自动开始导入
   （也可以先选多个文件，再点「导入图包」；未选文件时点「导入图包」会直接弹出文件选择框）。
3. 如需用新版替换已有插图，点击「插图更新」并选择新的 zip 图包；其中与现有完全同名的图片会被覆盖，其余旧图保留。
4. 导入或更新完成后，点击「预览插图」按钮，在全屏页面浏览当前角色的插图；悬停缩略图可删除。
5. 想一次性删除当前角色的全部插图时，点击「清空插图」并确认即可。
6. 想不打开角色卡就查看/整理其他角色的插图时，点击「管理全部插图」，在角色列表中进入对应角色即可浏览、删除、批量删除、改名或整角色清空。
7. 在对话中输入 `${图片名}`（例如 `${远坂凛}`），消息发送或生成后，占位符会自动替换为对应插图。
